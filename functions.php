...
function theretailer_styles() {	
	...
	wp_enqueue_style('theme-fonts', get_template_directory_uri() . '/fonts/theme-fonts/style.css', array(), '1.0', 'all' );
...
function theretailer_product_mod($atts, $content = null) {	
	extract(shortcode_atts(array(
		"id" => ''
	), $atts));
    ob_start();
...
add_shortcode("product_mod", "theretailer_product_mod");
...
function refresh_dynamic_contents() {
	global $woocommerce;
    $data = array(
        'cart_count_products' 				=> class_exists('WooCommerce') ? WC()->cart->get_cart_contents_count() : "",
        'cart_total' 						=> class_exists('WooCommerce') ? WC()->cart->get_cart_total() : "",
        'cart_count_products_with_label' 	=> class_exists('WooCommerce') ? sprintf( _n( '%d item', '%d items', WC()->cart->cart_contents_count, 'theretailer' ), WC()->cart->cart_contents_count ) : "",
    );
	wp_send_json($data);
}
...
function theretailer_ajax_added_to_cart() {
	...
	function theretailer_refresh_minicart_3($fragments) 
	{
        ...
        <a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="gbtr_little_shopping_bag_wrapper_mobiles"><span><?php echo WC()->cart->cart_contents_count; ?></span></a>
